...
const SECTIONS = [
  {
    key: "launch",
    title: "Launch",
    subtitle: "Build and launch new campaigns",
    icon: IconRocket,
    cols: { xs: 12, md: 6 },
    large: true,
  },
  {
    key: "optimise",
    title: "Optimise",
    subtitle: "Improve what\u2019s already running",
    icon: IconSettings,
  ...
  },
  {
    key: "scale",
    title: "Scale",
    subtitle: "Grow your best performers",
    icon: IconTrendingUp,
    cols: { xs: 12, sm: 6, md: 3 },
    large: false,
  },
...
function WorkflowCard({ workflow, large, accounts, onClick, index }) {
  ...
  const hasAccount = (accounts || []).some(
    (acc) => workflow.platform === "both" || acc.platform === workflow.platform,
  );
      ...
      <Box
        ...
        sx={{
          ...
          "&:hover": {
            ...
            boxShadow: `${glass.cardShadow}, 0 8px 32px rgba(128, 90, 245, 0.12)`,
          },
        }}
      >
        <Box
          sx={{ display: "flex", flexDirection: "column", height: "100%", p: large ? 3.5 : 2.5 }}
        >
          <Stack
            direction="row"
          ...
          >
            ...
            <Chip
              ...
              sx={{
                ...
                fontSize: 10.5,
                fontWeight: 700,
                bgcolor: `${platformConf.color}14`,
                color: platformConf.color,
                border: `1px solid ${platformConf.color}25`,
              }}
            />
          </Stack>
          <Typography variant={large ? "h5" : "h6"} sx={{ fontWeight: 700, mb: 0.75 }}>
            {workflow.title}
          </Typography>
          <Typography
            variant="body2"
            color="text.secondary"
            sx={{ mb: 2.5, flex: 1, lineHeight: 1.6, fontSize: 13.5 }}
          >
            ...
            <Stack direction="row" sx={{ alignItems: "center", gap: 0.75 }}>
              <Box
                sx={{
                  ...
                  height: 8,
                  borderRadius: "50%",
                  bgcolor: hasAccount ? "success.main" : "error.main",
                  boxShadow: hasAccount
                    ? "0 0 8px rgba(62,183,94,0.5)"
                    : "0 0 8px rgba(255,0,3,0.4)",
                }}
              />
              <Typography variant="caption" color="text.secondary" sx={{ fontSize: 12 }}>
                {hasAccount ? "Account connected" : "No account"}
              </Typography>
            </Stack>
            <Button
              size="small"
...
function WorkflowExecutionView({ workflow, accounts, onBack }) {
  ...
  useEffect(() => {
    const defaults = {};
    workflow.inputSchema.forEach((f) => {
  ...
  const handleRun = async () => {
    ...
    const missing = workflow.inputSchema.filter(
      (f) =>
        ...
        !(isDryRun && f.type === "account_select"),
    );
    if (missing.length > 0) {
      ...
      pollRef.current = setInterval(async () => {
        try {
          ...
          if (status?.status === "completed") {
            ...
            setProgress(Math.min(90, 10 + (elapsed / 300000) * 80));
  ...
  const handleExportJSON = () => {
    ...
    a.href = url;
    a.download = `${workflow.id}-result.json`;
    a.click();
  ...
  const renderField = (field) => {
    ...
    if (field.type === "account_select") {
      return (
        <Box key={field.key}>
          <InputLabel sx={{ mb: 0.5, color: "text.secondary", fontSize: 13 }}>
          ...
          {filteredAccounts.length === 0 && (
            <Typography variant="caption" sx={{ mt: 0.5, color: "warning.main", display: "block" }}>
              Connect an ad account via Settings
            </Typography>
          )}
        </Box>
      );
        ...
        <Stack direction="row" sx={{ alignItems: "center", gap: 1.5 }}>
          {filteredAccounts.length > 0 && (
            <Chip
              ...
              label={`${filteredAccounts.length} account${filteredAccounts.length > 1 ? "s" : ""}`}
              size="small"
              sx={{
      ...
      >
        ...
        <Box
          sx={{
            ...
            width: "100%",
            height: 3,
            background: `linear-gradient(90deg, ${platformConf.color}, ${platformConf.color}00)`,
          }}
        />
        <Stack direction="row" sx={{ alignItems: "center", gap: 3 }}>
          ...
          <Box sx={{ flex: 1 }}>
            <Stack
              direction="row"
              sx={{ alignItems: "center", gap: 1.5, mb: 0.5, flexWrap: "wrap" }}
            >
            ...
            <Typography
              variant="body1"
              color="text.secondary"
              sx={{ fontSize: 14.5, lineHeight: 1.6 }}
            >
              {workflow.description}
            </Typography>
          </Box>
          ...
          <Stack direction="row" sx={{ gap: 3, flexShrink: 0 }}>
            ...
            <Box sx={{ textAlign: "center" }}>
              ...
              >
                {workflow.section === "launch"
                  ? "Launch"
                  : workflow.section === "optimise"
                    ? "Opt"
                    : "Scale"}
              </Typography>
            </Box>
        ...
        {(phase === "input" || phase === "error") && (
          <Stack sx={{ gap: 3.5 }}>
            <Box>
              ...
              <Typography variant="body2" color="text.secondary">
                Fill in the details below and click Run to execute this workflow
              </Typography>
            </Box>
            ...
            <Stack direction="row" sx={{ gap: 2, alignItems: "center" }}>
              <Button
                ...
                sx={{
                  ...
                  "&:hover": { background: "linear-gradient(135deg, #9B7BF7 0%, #805AF5 100%)" },
                }}
              >
                Run Workflow
              </Button>
        ...
        {phase === "running" && (
          <Stack spacing={4} sx={{ alignItems: "center", py: 8, maxWidth: 600, mx: "auto" }}>
            ...
            <Box sx={{ textAlign: "center" }}>
              ...
              <Typography variant="body2" color="text.secondary">
                This may take up to 2 minutes depending on data volume
              </Typography>
            </Box>
            <Box sx={{ width: "100%" }}>
              <LinearProgress
              ...
              <Typography
                variant="caption"
                color="text.secondary"
                sx={{ mt: 0.75, display: "block", textAlign: "center", fontSize: 12 }}
              >
                {Math.round(progress)}%
              </Typography>
            </Box>
            ...
            <Stack direction="row" sx={{ alignItems: "center", justifyContent: "space-between" }}>
              <Stack direction="row" sx={{ alignItems: "center", gap: 1.5 }}>
                <Chip
                  ...
                  label={formData.dry_run !== false ? "Dry Run Complete" : "Workflow Complete"}
                  size="small"
                  sx={{
                    bgcolor:
                      formData.dry_run !== false
                        ? "rgba(255,200,118,0.12)"
                        : "rgba(62,183,94,0.12)",
                    color: formData.dry_run !== false ? "warning.main" : "success.main",
                    fontWeight: 600,
                    fontSize: 12,
                    height: 28,
              ...
              <Stack direction="row" spacing={0.5}>
                <IconButton
                  ...
                  sx={{ bgcolor: glass.surface, "&:hover": { bgcolor: "rgba(255,255,255,0.08)" } }}
                >
    ...
    <Stack sx={{ gap: { xs: 4, md: 5 } }}>
      ...
      <Box>
        ...
        <Typography variant="body1" color="text.secondary" sx={{ fontSize: 15 }}>
          Launch, optimise, and scale your ad campaigns with AI-powered workflows
        </Typography>
      </Box>
