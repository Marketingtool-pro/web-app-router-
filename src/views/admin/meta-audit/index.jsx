...
const AUDIT_CATEGORIES = [
  {
    ...
    desc: "Business Manager setup, verification & payment",
  },
  {
    key: "campaignPerformance",
    name: "Campaigns",
    icon: IconChartBar,
    desc: "CPA trends, ROAS & campaign structure",
  },
  {
    key: "adCreative",
    name: "Ad Creative",
    icon: IconPhoto,
    desc: "Fatigue, format diversity & A/B testing",
  },
  {
    key: "audienceTargeting",
    name: "Audiences",
    icon: IconUsers,
    desc: "Overlap, custom audiences & exclusions",
  },
  {
    key: "budgetBidding",
    name: "Budget & Bids",
    icon: IconCoin,
    desc: "Spend utilization, bid strategy & pacing",
  },
  {
    key: "conversionTracking",
    name: "Conversions",
    icon: IconTargetArrow,
    desc: "Pixel health, CAPI & attribution",
  },
];
  ...
  return (
    <GlassCard hover sx={{ height: "100%" }}>
      <Box sx={{ p: 2.5 }}>
        ...
        <Typography sx={{ fontWeight: 600, fontSize: "0.82rem", color: text.primary, mb: 0.75 }}>
          {name}
        </Typography>
        <LinearProgress
          variant="determinate"
    ...
    >
      ...
      >
        ...
        <Box sx={{ flex: 1, minWidth: 0 }}>
          ...
          <Typography sx={{ lineHeight: 1.6, fontSize: "0.8rem", color: text.secondary }}>
            {issue.description}
          </Typography>
        </Box>
        {issue.fix && (
        ...
        <Collapse in={expanded}>
          <Box sx={{ px: 2, pb: 2, pl: 7.5 }}>
            ...
            >
              ...
              <Typography sx={{ lineHeight: 1.7, fontSize: "0.8rem", color: text.secondary }}>
                {issue.fix}
              </Typography>
            </Box>
          </Box>
  ...
  return (
    ...
    >
      ...
      <Box sx={{ flex: 1, minWidth: 0 }}>
        ...
        <Typography
          sx={{ display: "block", lineHeight: 1.5, fontSize: "0.75rem", color: text.secondary }}
        >
          {rec.description}
        </Typography>
      </Box>
        ...
        <Box sx={{ p: { xs: 3, md: 6 }, textAlign: "center" }}>
          <Box sx={{ position: "relative", width: 80, height: 80, mx: "auto", mb: 3 }}>
            <CircularProgress
              size={80}
              thickness={1.5}
              sx={{ color: ACCENT, position: "absolute", opacity: 0.7 }}
            />
            <Box
              sx={{
                position: "absolute",
            ...
            {AUDIT_CATEGORIES.map((cat, i) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    {done ? (
                      <IconCircleCheck size={16} style={{ color: ACCENT }} />
                    ) : active ? (
                      <CircularProgress size={14} sx={{ color: ACCENT, opacity: 0.7 }} />
                    ) : (
                      <CatIcon size={15} style={{ color: text.muted }} />
                    )}
                  </Box>
                  <Typography
                    sx={{
                      fontWeight: active ? 600 : 400,
                      color: done || active ? text.primary : text.muted,
                      flex: 1,
                      fontSize: "0.88rem",
                    }}
                  >
                  ...
                  {done && (
                    <Typography
                      sx={{ color: ACCENT, fontWeight: 600, fontSize: "0.72rem", opacity: 0.8 }}
                    >
                      Done
                    </Typography>
                  )}
        ...
        <Box sx={{ p: { xs: 3, md: 5 }, position: "relative" }}>
          <Grid container spacing={4} sx={{ alignItems: "center" }}>
            <Grid size={{ xs: 12, md: 7 }}>
              ...
              >
                Most advertisers only see the surface. Our 360 audit analyzes 6 critical areas —
                uncovering hidden issues silently burning your budget.
              </Typography>
              ...
              {error && (
                <Alert
                  severity="error"
                  sx={{ mt: 2, maxWidth: 500, borderRadius: "12px" }}
            ...
            <Grid size={{ xs: 12, md: 5 }}>
              <Stack spacing={2.5}>
                {[
                  {
                    icon: IconEye,
                    label: "Hidden blind spots",
                    desc: "Issues invisible in Ads Manager",
                  },
                  {
                    icon: IconTrendingUp,
                    label: "Performance leaks",
                    desc: "Wasted spend & missed opportunities",
                  },
                  {
                    icon: IconChecklist,
                    label: "Prioritized action plan",
                    desc: "Know exactly what to fix first",
                  },
                ].map((item) => {
                  const F = item.icon;
                  return (
            ...
            <Grid key={cat.key} size={{ xs: 6, sm: 4, lg: 2 }}>
              <GlassCard hover sx={{ height: "100%" }}>
                <Box sx={{ p: 2.5, textAlign: "center" }}>
                  ...
                  <Typography
                    sx={{ fontWeight: 600, fontSize: "0.82rem", color: text.primary, mb: 0.5 }}
                  >
                    {cat.name}
                  </Typography>
                  <Typography sx={{ fontSize: "0.7rem", lineHeight: 1.45, color: text.muted }}>
                    {cat.desc}
                  </Typography>
                </Box>
              </GlassCard>
            ...
            {[
              {
                ...
                desc: "Paste your Meta Ad Account ID. We use read-only access — no changes to your account.",
              },
              {
                step: "02",
                title: "AI Analyzes 6 Areas",
                desc: "Our engine scans account health, campaigns, creative, targeting, budgets & tracking.",
              },
              {
                step: "03",
                title: "Get Actionable Report",
                desc: "Receive a scored report with critical issues, warnings & a prioritized action plan.",
              },
            ].map((item) => (
              <Grid key={item.step} size={{ xs: 12, md: 4 }}>
                <Stack sx={{ alignItems: "center", textAlign: "center" }}>
                  ...
                  <Typography
                    sx={{ fontWeight: 700, mb: 0.75, fontSize: "0.95rem", color: text.primary }}
                  >
                    {item.title}
                  </Typography>
                  <Typography
        ...
        <Box sx={{ p: { xs: 3, md: 4 } }}>
          ...
          <Grid container spacing={2}>
            {[
              {
                icon: IconTargetArrow,
                title: "Broken Pixel Events",
                desc: "Purchase events not firing on 30%+ of transactions, causing inaccurate ROAS reporting",
              },
              {
                icon: IconUsers,
                title: "Audience Overlap",
                desc: "Multiple ad sets competing against each other, inflating your CPMs by 15-30%",
              },
              {
                icon: IconPhoto,
                title: "Creative Fatigue",
                desc: "Ads with frequency above 3.5 burning budget on the same users repeatedly",
              },
              {
                icon: IconCoin,
                title: "Budget Underspend",
                desc: "Campaigns only using 40% of allocated budget due to narrow targeting or low bids",
              },
            ].map((issue) => {
              ...
              return (
                <Grid key={issue.title} size={{ xs: 12, sm: 6 }}>
                  ...
                  >
                    ...
                    <Box>
                      <Typography
                        sx={{ fontWeight: 600, fontSize: "0.85rem", color: text.primary, mb: 0.25 }}
                      >
                        {issue.title}
                      </Typography>
                      <Typography sx={{ lineHeight: 1.55, fontSize: "0.78rem", color: text.muted }}>
                        {issue.desc}
                      </Typography>
                    </Box>
                  </Stack>
      ...
      >
        <Box>
          ...
          <Typography sx={{ fontSize: "0.82rem", color: text.muted }}>
            {auditResult?.accountId || adAccountId} &middot;{" "}
            {new Date(auditResult?.auditDate || Date.now()).toLocaleDateString("en-US", {
              year: "numeric",
              month: "long",
              day: "numeric",
            })}
                ...
                <Grid key={stat.label} size={{ xs: 6, sm: 3 }}>
                  <GlassCard sx={{ height: "100%" }}>
                    <Box sx={{ p: 2 }}>
                      ...
                      <Typography sx={{ fontSize: "0.65rem", color: text.muted }}>
                        {stat.desc}
                      </Typography>
                    </Box>
                  </GlassCard>
          ...
          <Stack spacing={1.5}>
            {filteredIssues.length > 0 ? (
              ...
              <Box sx={{ py: 6, textAlign: "center" }}>
                ...
                <Typography sx={{ mt: 1, fontSize: "0.85rem", color: text.muted }}>
          ...
          <Box sx={{ p: 3 }}>
            ...
            <Stack>
              {recommendations.map((rec, i) => (
                <ActionItem key={i} rec={rec} index={i} />
              ))}
            </Stack>
          </Box>
        </GlassCard>
