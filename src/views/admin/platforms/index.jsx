...
const GOOGLE_IMAGES = Array.from(
  { length: 42 },
  (_, i) => `${IMG}/g-${String(i + 1).padStart(2, "0")}.jpg`,
);
...
function GoogleToolCard({ tool, image }) {
  ...
  return (
    ...
    >
      <Box
        sx={{ aspectRatio: "16/9", overflow: "hidden", position: "relative", bgcolor: "grey.900" }}
      >
        <img
      ...
      <Grid container spacing={2}>
        {tools.map((tool, idx) => (
          <Grid key={tool.slug} size={{ xs: 12, sm: 6, md: 4, lg: 3 }}>
            <GoogleToolCard
              tool={tool}
              image={GOOGLE_IMAGES[(startIndex + idx) % GOOGLE_IMAGES.length]}
            />
          </Grid>
        ))}
      </Grid>
...
// 39 unique images
const META_IMAGES = Array.from(
  { length: 39 },
  (_, i) => `${META_IMG}/m-${String(i + 1).padStart(2, "0")}.jpg`,
);
      ...
      <Grid container spacing={2}>
        {tools.map((tool, idx) => (
          <Grid key={tool.slug} size={{ xs: 12, sm: 6, md: 4, lg: 3 }}>
            <MetaToolCard
              tool={tool}
              image={META_IMAGES[(startIndex + idx) % META_IMAGES.length]}
            />
          </Grid>
        ))}
      </Grid>
      ...
      >
        ...
        <Box
          sx={{
            ...
            inset: 0,
            background:
              "linear-gradient(135deg, rgba(24,119,242,0.3) 0%, rgba(0,0,0,0.65) 50%, rgba(24,119,242,0.2) 100%)",
          }}
        />
      </Box>
...
const SOCIAL_IMAGES = Array.from(
  { length: 21 },
  (_, i) => `${INSTA_IMG}/i-${String(i + 1).padStart(2, "0")}.jpg`,
);

// Platform-specific images for each section
const YOUTUBE_IMAGES = Array.from(
  { length: 5 },
  (_, i) => `/images/tools/youtube/yt-${String(i + 1).padStart(2, "0")}.jpg`,
);
const TIKTOK_IMAGES = Array.from(
  { length: 4 },
  (_, i) => `/images/tools/tiktok/tt-${String(i + 1).padStart(2, "0")}.jpg`,
);
const LINKEDIN_IMAGES = Array.from(
  { length: 3 },
  (_, i) => `/images/tools/linkedin/li-${String(i + 1).padStart(2, "0")}.jpg`,
);

// 16 social media management images
const SM_IMG = "/images/tools/social-media";
const SOCIAL_MGMT_IMAGES = Array.from(
  { length: 16 },
  (_, i) => `${SM_IMG}/sm-${String(i + 1).padStart(2, "0")}.jpg`,
);
            ...
            <SocialToolCard
              ...
              image={
                sectionImages
                  ...
                  : SOCIAL_IMAGES[(startIndex + idx) % SOCIAL_IMAGES.length]
              }
            />
          </Grid>
        ))}
      ...
      >
        ...
        <Box
          sx={{
            ...
            inset: 0,
            background:
              "linear-gradient(135deg, rgba(225,48,108,0.3) 0%, rgba(0,0,0,0.65) 50%, rgba(124,58,237,0.2) 100%)",
          }}
        />
      </Box>
...
const PLATFORM_CONFIG = {
  google: {
    ...
    filterFn: (tool) => {
      ...
      if (
        ...
        tool.badge === "Facebook/Meta" ||
        tool.badge === "Instagram" ||
        tool.badge === "TikTok" ||
        tool.badge === "LinkedIn" ||
        tool.badge === "E-commerce"
      ) {
        return false;
      }
      if (name.includes("google") || slug.includes("google") || tool.badge === "Google Ads")
        return true;
      if (
        tool.badge === "Campaign" ||
        tool.badge === "Budget" ||
        tool.badge === "PPC Optimization" ||
        tool.badge === "Audit"
      )
        return true;
      return false;
    ...
    sections: [
      {
        key: "grader",
        title: "Google Graders",
        subtitle: "Score and analyze your Google accounts",
        icon: IconChartBar,
        filter: (t) =>
          (t.badge === "Grader" && t.name.toLowerCase().includes("google")) ||
          t.badge === "Google Ads",
      },
      {
        key: "audit",
        title: "Audit & Analysis",
        subtitle: "Diagnose issues and find opportunities",
        icon: IconShieldCheck,
        filter: (t) => t.badge === "Audit",
      },
      {
        key: "campaign",
        title: "Campaign Management",
        subtitle: "Build, manage and optimize campaigns",
        icon: IconRocket,
        filter: (t) => t.badge === "Campaign",
      },
      {
        key: "budget",
        title: "Budget & Bidding",
        subtitle: "Control spend and maximize ROI",
        icon: IconCoin,
        filter: (t) => t.badge === "Budget" || t.badge === "PPC Optimization",
      },
    ],
  },
  meta: {
  ...
  instagram: {
    ...
    filterFn: (tool) => {
      const name = tool.name.toLowerCase();
      return (
        name.includes("instagram") || tool.badge === "Instagram" || tool.badge === "Social Media"
      );
    },
    sections: [
      {
        key: "instagram",
        title: "Instagram",
        subtitle: "Reels, Stories & Engagement",
        icon: IconBrandInstagram,
    ...
    sections: [
      {
        key: "youtube",
        title: "YouTube",
        subtitle: "Video ads and content",
        icon: IconBrandYoutube,
        filter: (t) => t.badge === "YouTube" || t.name.toLowerCase().includes("youtube"),
      },
    ],
  },
  tiktok: {
    ...
    sections: [
      {
        ...
        icon: IconBrandTiktok,
        filter: (t) => t.badge === "TikTok" || t.name.toLowerCase().includes("tiktok"),
      },
    ],
  },
  linkedin: {
    ...
    sections: [
      {
        key: "linkedin",
        title: "LinkedIn",
        subtitle: "B2B marketing and lead gen",
        icon: IconBrandLinkedin,
        filter: (t) => t.badge === "LinkedIn" || t.name.toLowerCase().includes("linkedin"),
      },
    ],
  },
  seo: {
    ...
    icon: IconSearch,
    filterFn: (tool) =>
      tool.badge === "SEO" || tool.badge === "Content Writing" || tool.badge === "Copywriting",
    sections: [
      {
        key: "seo",
        title: "SEO",
      ...
      },
      {
        ...
        icon: IconPencil,
        filter: (t) => t.badge === "Content Writing" || t.badge === "Copywriting",
      },
    ],
  },
  analytics: {
    ...
    sections: [
      {
        ...
        icon: IconChartBar,
        filter: (t) => t.badge === "Analytics",
      },
      {
        key: "roi",
        title: "ROI & Attribution",
        subtitle: "Track conversions and measure campaign returns",
        icon: IconTrendingUp,
        filter: (t) => t.badge === "ROI & Attribution",
      },
    ],
  },
  ecommerce: {
    ...
    filterFn: (tool) => {
      return (
        tool.badge === "E-commerce" ||
        tool.badge === "Shopify" ||
        (tool.badge === "Campaign" && tool.name.toLowerCase().includes("shopify"))
      );
    },
    sections: [
      {
        key: "shopify",
        title: "Shopify Tools",
        subtitle: "Store optimization, marketing, and conversion tools for Shopify",
        icon: IconBriefcase,
        accent: "#4285F4",
      ...
      },
      {
        ...
        subtitle: "Product ads, shopping campaigns, retargeting, and budget optimization",
        icon: IconRocket,
        accent: "#4285F4",
        filter: (t) => {
          const s = t.slug;
      ...
      },
      {
        ...
        subtitle: "KPI tracking, ROAS analysis, conversion optimization, and reporting",
        icon: IconChartBar,
        accent: "#4285F4",
        filter: (t) => {
          const s = t.slug;
      ...
      {
        key: "ecommerce-automation",
        title: "E-commerce Automation",
        subtitle: "Email flows, loyalty programs, replenishment, and workflow automation",
        icon: IconSettings,
        accent: "#4285F4",
        filter: (t) => {
          const s = t.slug;
      ...
      {
        key: "ecommerce-strategy",
        title: "E-commerce Strategy",
        subtitle: "Growth planning, pricing, inventory forecasting, and platform expansion",
        icon: IconTrendingUp,
        accent: "#4285F4",
        filter: (t) => {
          const s = t.slug;
          const strategySlugs = [
            ...
            "platform-expansion-planner",
            "ecommerce-ab-test-planner",
    ...
    sections: [
      {
        key: "ai-agents",
        title: "AI Agents",
        subtitle: "Autonomous AI marketing agents and intelligent automation",
        icon: IconRobot,
        accent: "#4285F4",
        filter: (t) => t.badge === "AI Agent",
      },
      {
        key: "marketing",
        title: "Marketing & Advertising",
        subtitle: "Marketing copy, growth tools, ad analysis, and brand assets",
        icon: IconTrendingUp,
        accent: "#4285F4",
        filter: (t) => ["Marketing", "Advertising", "Branding"].includes(t.badge),
      },
      {
        key: "text-editing",
        title: "Text & Content Editing",
        subtitle: "Rewrite, expand, simplify, and improve any text",
        icon: IconPencil,
        accent: "#4285F4",
        filter: (t) => t.badge === "Text Editing",
      },
      {
        key: "developer",
        title: "Developer & Automation",
        subtitle: "API documentation, schemas, and workflow automation",
        icon: IconCode,
        accent: "#4285F4",
        filter: (t) => ["Developer", "Schema", "Automation"].includes(t.badge),
      },
      {
        key: "communication",
        title: "Email, Graders & More",
        subtitle: "Email templates, scoring tools, education, and cross-platform marketing",
        icon: IconMail,
        accent: "#4285F4",
        filter: (t) => ["Email", "Grader", "Education", "Pinterest", "Twitter/X"].includes(t.badge),
      },
    ],
  },
};
...
const SEO_IMAGES = Array.from(
  { length: 11 },
  (_, i) => `${SEO_IMG}/seo-${String(i + 1).padStart(2, "0")}.jpg`,
);
const CONTENT_IMAGES = Array.from(
  { length: 15 },
  (_, i) => `${CONTENT_IMG}/ct-${String(i + 1).padStart(2, "0")}.jpg`,
);
      ...
      >
        ...
        <Box
          sx={{
            ...
            inset: 0,
            background:
              "linear-gradient(135deg, rgba(16,185,129,0.3) 0%, rgba(0,0,0,0.65) 50%, rgba(139,92,246,0.2) 100%)",
          }}
        />
      </Box>
      <Stack
...
const ANALYTICS_IMAGES = Array.from(
  { length: 13 },
  (_, i) => `${ANALYTICS_IMG}/an-${String(i + 1).padStart(2, "0")}.jpg`,
);
const ROI_IMAGES = Array.from(
  { length: 6 },
  (_, i) => `${ROI_IMG}/roi-${String(i + 1).padStart(2, "0")}.jpg`,
);
      ...
      >
        ...
        <Box
          sx={{
            ...
            inset: 0,
            background:
              "linear-gradient(135deg, rgba(66,133,244,0.3) 0%, rgba(0,0,0,0.65) 50%, rgba(139,92,246,0.2) 100%)",
          }}
        />
      </Box>
      <Stack
...
const ECOMMERCE_IMAGES = Array.from(
  { length: 49 },
  (_, i) => `${EC_IMG}/ec-${String(i + 1).padStart(2, "0")}.jpg`,
);
...
const AI_IMAGES = Array.from(
  { length: 57 },
  (_, i) => `${AI_IMG}/ai-${String(i + 1).padStart(2, "0")}.jpg`,
);
  ...
  return (
    <Box
      ...
      sx={{
        ...
        transition: "all 0.3s cubic-bezier(0.4, 0, 0.2, 1)",
        "&:hover": {
          borderColor: `${AI_ACCENT}66`,
          transform: "translateY(-4px)",
          boxShadow: `0 12px 40px ${AI_ACCENT}1F`,
        },
    ...
    <Box sx={{ position: "relative" }}>
      ...
      >
        {sectionsWithTools
          .reduce((acc, section) => {
            const offset =
              acc.length === 0 ? 0 : acc[acc.length - 1].offset + acc[acc.length - 1].count;
