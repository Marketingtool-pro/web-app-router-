...
// @project
import { deleteUser, handleUserFilter, initialUserFilter, useGetUserFilter } from '../api';
...
export default function Toolbar({ table, onGlobalSearch, filterHeight }) {
  ...
  const onDelete = async () => {
    ...
    await deleteUser(selectedIds).then(() => console.log('Users deleted - ', selectedIds));
      ...
      <Stack direction="row" sx={{ alignItems: 'center', justifyContent: 'space-between', gap, minWidth: { xs: 1, sm: 'fit-content' } }}>
        <Stack direction="row" sx={{ alignItems: 'center', gap }}>
          ...
          <DebouncedInput
            ...
            value=""
            onValueChange={(data) => {
              if (globalFilter) handleUserFilter({ ...userFilter, globalFilter: data.toString() });
              onGlobalSearch(data);
            }}
            borderless
          />
      ...
      >
        ...
        {((columnFilters && columnFilters.length > 0) || globalFilter) && (
          <>
            ...
            >
              {globalFilter && (
                <Chip
                  ...
                  onDelete={() => handleUserFilter({ ...userFilter, globalFilter: '' })}
                />
              )}
                ...
                <Fragment key={index}>
                  {typeof filter.value === 'string' && (
                    <Chip
                      ...
                      variant="tag"
                      sx={{ '&.Mui-focusVisible': { boxShadow: 'none', borderColor: 'grey.600' } }}
                      onDelete={() => handleUserFilter({ ...userFilter, columnFilters: columnFilters.filter((f) => f !== filter) })}
                    />
                  )}
                  {Array.isArray(filter.value) &&
                    filter.value.map((item, index) => (
                      <Chip
                        ...
                        variant="tag"
                        sx={{ '&.Mui-focusVisible': { boxShadow: 'none', borderColor: 'grey.600' } }}
                        onDelete={() => handleUserFilter(removeFilterItem(userFilter, filter, item))}
                      />
                    ))}
                        ...
                        ([key, value]) =>
                          value !== null && (
                            <Chip
                              ...
                              onDelete={() => handleUserFilter(removeFilterItem(userFilter, filter, value))}
                            />
                          )
                      )}
                    </>
            ...
            <Button color="error" sx={{ display: { xs: 'none', sm: 'block' } }} onClick={() => handleUserFilter(initialUserFilter)}>
              Clear All
            </Button>
            <IconButton
              color="error"
              ...
              aria-label="delete"
              onClick={() => handleUserFilter(initialUserFilter)}
            >
              <IconX size={16} />
            </IconButton>
          </>
