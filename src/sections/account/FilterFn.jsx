...
export function removeFilterItem(accountFilter, filter, item) {
  const updatedFilters = accountFilter?.columnFilters
    .map((f) => {
      if (f.id === filter.id) {
  ...
  return { ...accountFilter, columnFilters: updatedFilters };
}
