...
// @project
import { deleteAccount, handleAccountFilter, initialAccountFilter, useGetAccountFilter } from '../api';
...
export default function Toolbar({ table, onGlobalSearch, filterHeight }) {
  ...
  const { columnFilters, globalFilter } = accountFilter || {};
  ...
  const onDelete = async () => {
    ...
    await deleteAccount(selectedIds).then(() => console.log('Accounts deleted - ', selectedIds));
      ...
      <Stack direction="row" sx={{ alignItems: 'center', justifyContent: 'space-between', gap, minWidth: { xs: 1, sm: 'fit-content' } }}>
        <Stack direction="row" sx={{ alignItems: 'center', gap }}>
          ...
          <DebouncedInput
            ...
            value=""
            onValueChange={(data) => {
              if (globalFilter) handleAccountFilter({ ...accountFilter, globalFilter: data.toString() });
              onGlobalSearch(data);
            }}
            borderless
          />
      ...
      >
        ...
        {((columnFilters && columnFilters.length > 0) || globalFilter) && (
          <>
            ...
            >
              {globalFilter && (
                <Chip
                  ...
                  onDelete={() => handleAccountFilter({ ...accountFilter, globalFilter: '' })}
                />
              )}
                ...
                <Fragment key={index}>
                  {typeof filter.value === 'string' && (
                    <Chip
                      ...
                      variant="tag"
                      sx={{ '&.Mui-focusVisible': { boxShadow: 'none', borderColor: 'grey.600' } }}
                      onDelete={() => handleAccountFilter({ ...accountFilter, columnFilters: columnFilters.filter((f) => f !== filter) })}
                    />
                  )}
                  {Array.isArray(filter.value) &&
                    filter.value.map((item, index) => (
                      <Chip
                        ...
                        variant="tag"
                        sx={{ '&.Mui-focusVisible': { boxShadow: 'none', borderColor: 'grey.600' } }}
                        onDelete={() => handleAccountFilter(removeFilterItem(accountFilter, filter, item))}
                      />
                    ))}
                        ...
                        ([key, value]) =>
                          value !== null && (
                            <Chip
                              ...
                              onDelete={() => handleAccountFilter(removeFilterItem(accountFilter, filter, value))}
                            />
                          )
                      )}
                    </>
            ...
            <Button color="error" sx={{ display: { xs: 'none', sm: 'block' } }} onClick={() => handleAccountFilter(initialAccountFilter)}>
              Clear All
            </Button>
            <IconButton
              color="error"
              ...
              aria-label="delete"
              onClick={() => handleAccountFilter(initialAccountFilter)}
            >
              <IconX size={16} />
            </IconButton>
          </>
