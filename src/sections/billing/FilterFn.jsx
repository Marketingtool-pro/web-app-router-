...
export function removeFilterItem(billingFilter, filter, item) {
  const updatedFilters = billingFilter?.columnFilters
    .map((f) => {
      if (f.id === filter.id) {
  ...
  return { ...billingFilter, columnFilters: updatedFilters };
}
