...
export default function ActionCell({ row, onDelete }) {
  ...
  const handleDialogDelete = (id) => {
    startTransition(async () => {
      ...
      // Example: await deletePricingPlan(id);
      await new Promise((resolve) => setTimeout(() => resolve('ok'), 3000));
      enqueueSnackbar(`Pricing plan has been deleted.`, { variant: 'success' });
      onDelete(id);
      setOpenDeleteDialog(false);
    });
  };
                ...
                <motion.div variants={varSlide('slideInUp', { distance: 20 })} initial="initial" animate="animate" exit="exit">
                  <MainCard sx={{ borderRadius: 3, boxShadow: theme.vars.customShadows.tooltip, minWidth: 150, p: 0.5 }}>
                    <ClickAwayListener onClickAway={() => setAnchorEl(null)}>
                      <List disablePadding>
                        <ListItemButton sx={buttonStyle} component={RouterLink} to={`/setting/pricing/plan-edit/${row.id}`}>
                          <ListItemIcon>
